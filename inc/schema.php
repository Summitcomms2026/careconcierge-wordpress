...
/**
 * Canonical FAQ source. Mirrors the visible accordion in
 * patterns/section-03-solution.php exactly — the answer text below
 * is verbatim from those rendered <p> elements. If the visible copy
 * is ever edited, update this list to keep schema and page in sync.
 *
 * @return array<int, array{question:string,answer:string}>
 */
if ( ! function_exists( 'careconcierge_faq_source' ) ) {
	function careconcierge_faq_source() {
		return array(
			array(
...
/**
 * Build the FAQPage schema node from the canonical FAQ source.
 * Mirrors the visible accordion verbatim so schema and page stay in sync.
 */
if ( ! function_exists( 'careconcierge_schema_faqpage' ) ) {
	function careconcierge_schema_faqpage() {
		$entities = array();
		foreach ( careconcierge_faq_source() as $i => $row ) {
			$entities[] = array(
				'@type'          => 'Question',
				'name'           => $row['question'],
		...
		return array(
			'@type'      => 'FAQPage',
			'@id'        => CARECONCIERGE_CANONICAL_HOST . '/surgeons/#faq',
			'mainEntity' => $entities,
		);
	}
}
...
if ( ! function_exists( 'careconcierge_print_jsonld' ) ) {
	function careconcierge_print_jsonld() {
		...
		$is_master = in_array( $path, array( '/', '/surgeons', '/surgeons/' ), true );
		...
		/* Service / FAQPage / DigitalDocument are tied to the master
		   page composition. They are emitted only on URLs that render
		   that composition (home + /surgeons/) — emitting them on
		   unrelated pages would misrepresent those pages' content. */
		if ( $is_master ) {
